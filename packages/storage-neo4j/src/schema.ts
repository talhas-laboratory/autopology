...
const DDL = [
  ...
  'CREATE CONSTRAINT warm_cache_key_unique IF NOT EXISTS FOR (w:WarmCache) REQUIRE w.key IS UNIQUE',
...
export async function initSchema(ctx: Neo4jContext, repoRoot: string): Promise<void> {
  ...
  try {
    ...
    await session.run(
      `MERGE (r:CodeNode:Repo {id: 'repo:root'})
       SET r.path = $repoRoot,
           r.name = 'repo',
